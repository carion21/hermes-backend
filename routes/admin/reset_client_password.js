...
const { APP_NAME, DEFAULT_PASSWORD_BACKOFFICE, USERPROFILE_TYPE_CLIENT } = require('../../config/consts');
...
router.post('/', async function (req, res, next) {
  ...
  if (hcontrol.success) {
    ...
    if (bcontrol.success) {
      ...
      if (client) {
        client.password = getHashpass(DEFAULT_PASSWORD_BACKOFFICE)
        await client.save()

        result.success = true
        result.message = "Client password reseted with success"
      } else {
