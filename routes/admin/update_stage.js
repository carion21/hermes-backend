...
const SERVICE_TYPE = "admin_update_stage"
const hms = getModel()
...
router.post('/', async function (req, res, next) {
  ...
  if (hcontrol.success) {let body = req.body
    ...
    if (bcontrol.success) {
      ...
      if (stage) {
        ...
        stage.description = body.description || stage.description
        stage.parent_stage_id = body.parent_stage_id || stage.parent_stage_id
        await stage.save()

        result.success = true
        result.message = "Stage updated with success"
      } else {
